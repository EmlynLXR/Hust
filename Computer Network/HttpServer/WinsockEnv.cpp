...
int WinsockEnv::Startup(std::thread::id threadid) {
	...
	if (rtn) { //如果返回值不是0，则Winsock初始化失败
		id = WSAGetLastError();
		printf("[Thread_id：%d]Winsock startup error! [WSAGetLastError() :%d]", threadid,id);
		return -1;
	}
	...
	if (LOBYTE(wsaData.wVersion) != MAJORVERSION || HIBYTE(wsaData.wVersion) != MINORVERSION) { //如果版本不对
		...
		id = WSAGetLastError();
		printf("[Thread_id：%d]Winsock version error! [WSAGetLastError() :%d]",threadid, id);
		return -1;
	}

	printf("[Thread_id：%d]Winsock startup ok!\n",threadid);
	return 0;
}
