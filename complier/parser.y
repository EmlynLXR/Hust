...
%type  <ptr> ForFirVar ForSecVar ForThiVar

//% token 定义终结符的语义值类型
%token <type_int> INT              //指定INT的语义值是type_int，有词法分析得到的数值
%token <type_id> ID RELOP TYPE COMOP DOUOP  //指定ID,RELOP 的语义值是type_id，有词法分析得到的标识符字符串
...
program: ExtDefList    { semantic_Analysis0($1);/*display($1,0);*/ }     /*显示语法树,语义分析*/
         ; 
ExtDefList: {$$=NULL;}
...
ExtDef:   Specifier ExtDecList SEMI   {$$=mknode(EXT_VAR_DEF,$1,$2,NULL,yylineno);}   //该结点对应一个外部变量声明
         ...
         | error SEMI   {$$=NULL; }
         ;
Specifier:  TYPE    {$$=mknode(TYPE,NULL,NULL,NULL,yylineno);strcpy($$->type_id,$1);$$->type=!strcmp($1,"int")?INT:(!strcmp($1,"float")?FLOAT:CHAR);}   //符
           ;      
...
Stmt:   Exp SEMI    {$$=mknode(EXP_STMT,$1,NULL,NULL,yylineno);}
      ...
      | FOR LP ForFirVar RP Stmt {$$=mknode(FOR,$3,$5,NULL,yylineno);}
      
      ;

ForFirVar : Exp SEMI ForSecVar   {$$=mknode(ForFirVar,$1,$3,NULL,yylineno);}
        | SEMI ForSecVar {$$=mknode(ForFirVar,NULL,$2,NULL,yylineno);}
        ;
ForSecVar : Exp SEMI ForThiVar {$$=mknode(ForSecVar,$1,$3,NULL,yylineno);}
        | SEMI ForThiVar {$$=mknode(ForSecVar,NULL,$2,NULL,yylineno);}
        ;
ForThiVar : Exp {$$=mknode(ForThiVar,$1,NULL,NULL,yylineno);}  
        |  {$$=mknode(ForThiVar,NULL,NULL,NULL,yylineno);}  
        ;
...
Exp:    Exp ASSIGNOP Exp {$$=mknode(ASSIGNOP,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGNOP");}//$$结点type_id空置未用，正好存放运算符
      ...
      | Exp COMOP Exp {$$=mknode(COMOP,$1,$3,NULL,yylineno);strcpy($$->type_id,$2);}  //词法分析复合赋值运算符号自身值保存在$2中
      ...
      | DOUOP Exp     {$$=mknode(PREOP,$2,NULL,NULL,yylineno);strcpy($$->type_id,$1);} //前置自增 
      | Exp DOUOP     {$$=mknode(AFTOP,$1,NULL,NULL,yylineno);strcpy($$->type_id,$2);} //后置自增
